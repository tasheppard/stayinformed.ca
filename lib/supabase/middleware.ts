...
export async function updateSession(request: NextRequest) {
  ...
  if (!supabaseUrl || !supabaseAnonKey) {
    ...
    // This prevents routing errors when env vars are missing
  ...
  try {
    ...
    // Protect premium routes (will be enhanced in task 8.0)
    // For now, we just check if user is authenticated
    // Premium check will be done in the actual route handlers
    if (
      !user &&
